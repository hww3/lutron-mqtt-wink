...
werror("got device id\n");
    ...
    else if(json->cmd == "DeleteColumnProgrammingResponse") {
      werror("remote unpairing response: %s\n", (json->args->Status?"FAILURE": "SUCCESS"));
      exit(json->args->Status);
  }

     if(!gotit && nameid && remoteid) {
       werror("got ids for our pair.\n");
string payload = Standards.JSON.encode((["cmd": "RequestDeleteColumnProgramming", "args": (["ButtonGroupID": remoteid, "AssignableObjectID": nameid, "AssignableObjectType": 15])]));
