...
call_out(unaddress, 10);
}                           

void unaddress() {
  mqtt->publish("lutron/commands", Standards.JSON.encode((["cmd": "RequestUnaddressDevice", "args": (["SerialNumber": (int)name])])), 0, pub_failed);
  call_out(exit_addressing_mode,2);
}
